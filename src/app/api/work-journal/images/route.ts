...
async function currentEmployee(authUid: string) {
  ...
  const { data } = await admin.from("employees").select("id").eq("auth_uid", authUid).maybeSingle();
  return data?.id ? { admin, employeeId: data.id as string } : null;
}
...
export async function POST(request: NextRequest) {
  ...
  const storagePath = `${current.employeeId}/board/${crypto.randomUUID()}.${extension}`;
...
export async function DELETE(request: NextRequest) {
  ...
  if (!image || image.employee_id !== current.employeeId) return NextResponse.json({ error: "이미지를 찾을 수 없습니다." }, { status: 404 });
